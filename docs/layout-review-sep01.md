...
`PL.5b_LV` and both waived, bbox **1110 × 550 µm** with the 5-shape `0/0` boundary, all 25
top-cell labels (14 on `36/10`) byte-identical in name, layer and position to the committed
`chip_top`, and metal2 corner obstruction identical (SW 218.24 µm², NW 219.90 µm², SE/NE empty).
...
| top-cell labels | 25, 14 on `36/10` | **identical in name, layer and position** |
| bbox / `0/0` boundary | 1110 × 550 µm, 5 shapes | **unchanged** |

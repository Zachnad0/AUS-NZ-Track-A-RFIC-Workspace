...
| **DIV2 (`DIV2_QUAD_v1`) (5.3)** | 🟡 **IN PROGRESS** | Schematic re-frozen (`gen_div2_quad.py` → `.sch`, commit `f600af3`). **CML latch PASSING** (Magic+KLayout DRC 0, LVS match uniquely, 9 ports). Converter sub-blocks all validated (diff-pair front-end, INV1/2/3, all FET widths, R/C). Remaining: converter assembly (1e) + full `DIV2_QUAD_v1` assembly (1f) |

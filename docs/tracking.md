...
| **DIV2 (`DIV2_QUAD_v1`) (5.3)** | 🟡 **IN PROGRESS** | Schematic re-frozen (`gen_div2_quad.py` → `.sch`, commit `f600af3`). **CML latch (1d) + full CONVERTER (1e) both GATE-PASSED** — `ib_conv_v1` 14 devices / 6 ports, magic DRC 0, KLayout DRC 0 (variant D), LVS match uniquely. RFB laid out serpentine (nx2 l20.6 = 20k, r_length 40.04u). Remaining: **1f** full `DIV2_QUAD_v1` (2 latches + 3 NMOS bias + 4 converters, 9 ports, golden via `mk_div2_golden.py`) |

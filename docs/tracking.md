...
| **DIV2 (`DIV2_QUAD_v1`) (5.3)** | 🟡 **IN PROGRESS — 1f Stage A CLOSED** | **Stage A (2 CML latches + 3 NMOS bias, 19 devices) GATE-PASSED**: `ib_div2` magic DRC 0 + KLayout DRC 0 (var-D) + LVS match uniquely vs the 19-device partial golden, 9 ports. Core+bias bbox **120.4 × 60.1 µm ≈ 7,236 µm²**. Remaining: **Stages B/C** — add 4 converters (ib_conv_v1 recipe: IP=OIB/OI/I_P, IN=OI/OIB/I_N, QP=OQB/OQ/Q_P, QN=OQ/OQB/Q_N) → 75-device golden — and **D** (9 ports CK CKB IBIAS I_P I_N Q_P Q_N VDD VSS, `verify_cp.sh DIV2_QUAD_v1` exit 0, closes 5.3). Next step: refactor `ib_conv_v1` into a `place_conv {cx cy tag}` proc (like `route_latch`) and call it 4× flat. Golden via `mk_div2_golden.py` (netlist with PDK xschemrc). |
